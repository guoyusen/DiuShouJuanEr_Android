package com.bili.diushoujuaner.widget.stepview;
...
public class StepsView extends LinearLayout implements StepsViewIndicator.OnDrawListener {
    ...
    private void init() {
        ...
        View rootView = LayoutInflater.from(getContext()).inflate(R.layout.layout_steps_view, this);
        ...
        txtPaint.setTextSize(getContext().getResources().getDimension(R.dimen.x16));
    ...
    private void drawLabels() {
        ...
        if (mLabels != null) {
            for (int i = 0, len = mLabels.length; i < len; i++) {
                TextView textView = new TextView(getContext());
                textView.setText(mLabels[i]);
                textView.setTextSize(getContext().getResources().getDimension(R.dimen.x10));
                textView.setTextColor(mLabelColorIndicator);
                textView.setTypeface(null, Typeface.BOLD);
                if(i == 0){
                    textView.setX(indicatorPosition.get(i));
                }else if(i == len - 1){
                    textView.setX((float)(indicatorPosition.get(i) - Math.ceil(Layout.getDesiredWidth(mLabels[i], txtPaint))));
                }else{
                    textView.setX((float)(indicatorPosition.get(i) - Math.ceil(Layout.getDesiredWidth(mLabels[i], txtPaint)) / 2));
                }
