 ...
 */
public class RecallAddActivity extends BaseActivity<RecallAddActivityPresenter> implements IBaseView, View.OnClickListener {
    ...
    public void setViewStatus() {
        ...
        recallAddPicVo.setType(ConstantUtil.RECALL_ADD_PIC_RES);
        ...
        gridview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if(picVoList.get(position).getType() == ConstantUtil.RECALL_ADD_PIC_RES){
                ...
                }else if(picVoList.get(position).getType() == ConstantUtil.RECALL_ADD_PIC_PATH){
                    recallAddPicAdapter.remove(position);
                    images.remove(position);
                    //小于9张，且最后一张不是add，添加add图标
                    if(recallAddPicAdapter.getCount() < 9 && recallAddPicAdapter.getItem(recallAddPicAdapter.getCount() - 1).getType() != ConstantUtil.RECALL_ADD_PIC_RES){
                        RecallAddPicVo recallAddPicVo = new RecallAddPicVo();
                        recallAddPicVo.setType(ConstantUtil.RECALL_ADD_PIC_RES);
                        recallAddPicVo.setResourceId(R.mipmap.icon_recall_add);
                        picVoList.add(recallAddPicVo);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == ImagePicker.RESULT_CODE_ITEMS && data != null && requestCode == 100) {
            ...
            for(ImageItemVo item : images){
                RecallAddPicVo recallAddPicVo = new RecallAddPicVo();
                recallAddPicVo.setType(ConstantUtil.RECALL_ADD_PIC_PATH);
