...
public class PartyAdapter extends CommonAdapter<PartyVo> {
    ...
    public void convert(ViewHolder holder, PartyVo partyVo, int position) throws Exception {
        if(partyVo != null){
            ...
            Imageloader.getInstance().displayDraweeView(Common.getCompleteUrl(partyVo.getPicPath()), (SimpleDraweeView) holder.getView(R.id.ivItemHead));
            ((TextView) holder.getView(R.id.txtContactName)).setText(partyVo.getDisplayName());
        }
    }
